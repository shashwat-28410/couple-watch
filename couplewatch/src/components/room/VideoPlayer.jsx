...
export function VideoPlayer({ 
  ...
  const handleVideoClick = (e) => {
    ...
    controlsTimeoutRef.current = setTimeout(() => {
        ...
        if (!e.target.closest('.adjustments-panel')) setShowAdjustments(false);
    }, 5000);
  ...
  return (
    <div ref={containerRef} className={`romantic-card !p-0 border-white/5 bg-white/[0.02] overflow-hidden shadow-2xl relative ${isFullScreen ? 'fixed inset-0 z-[100] rounded-none' : ''}`}>
      <div className={`relative bg-black group cursor-pointer overflow-hidden ${isFullScreen ? 'w-full h-full' : 'aspect-video'}`} onClick={handleVideoClick}>
        {floatingReactions.map(r => (
          <div key={r.id} className="float-reaction" style={{ left: `${r.left}%`, bottom: '20px' }}>{r.emoji}</div>
        ))}
        {isScreenShare ? (
          <video ref={streamVideoRef} className="absolute inset-0 w-full h-full object-contain" style={videoStyle} playsInline autoPlay muted={isHost} />
        ) : roomState?.video_url ? (
          <video 
            ref={playerRef} src={roomState.video_url} className="absolute inset-0 w-full h-full object-contain" style={videoStyle} playsInline
